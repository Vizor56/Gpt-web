...
async function buildStaffWorkspace(staff) {
  ...
  return {
    ...
    staffShop: await getStaffShopPayload(staff),
    notifications: buildStaffNotifications(staff, [...mappedHomeworks, ...mappedCorrections], [...mappedStats, ...mappedCorrectionStats]),
  };
}
...
function buildStaffNotifications(staff, homeworks, stats) {
  ...
  if (staff.role === "Teacher") {
    return homeworks
      ...
      .map((homework) => {
        ...
        return {
          notificationId: `${isCorrection ? "teacher-correction" : "teacher-submission"}-${homework.homeworkSubmissionId}`,
          type: isCorrection ? "CorrectionSubmitted" : "HomeworkSubmitted",
  ...
  return homeworks
    ...
    .map((homework) => {
      ...
      const isCorrection = Boolean(homework.isCorrection);
      return {
        notificationId: `${isCorrection ? "curator-correction" : "curator-checked"}-${homework.homeworkSubmissionId}`,
        type: isCorrection ? "CorrectionChecked" : "HomeworkChecked",
  ...
  asyncRoute(async (request, response) => {
    ...
    try {
      ...
      if (score < 5) {
        const correction = await client.query(
          `
            INSERT INTO homework_corrections (assignment_id, student_id, teacher_id, enrollment_id, required_score, status)
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    ...
    try {
      ...
      await client.query("COMMIT");
      response.json({ ok: true, source: "database", correctionId, homeworkAssignmentId: row.assignment_id, score, feedbackText });
    } catch (error) {
      await client.query("ROLLBACK");
      throw error;
    } finally {
