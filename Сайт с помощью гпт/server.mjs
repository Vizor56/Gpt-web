...
function lessonPassedSql(assignmentAlias = "ha", submissionAlias = "hs") {
  return `(COALESCE(${passedCheckedSql(submissionAlias)}, FALSE) OR ${correctionPassedExistsSql(assignmentAlias)})`;
}
...
async function buildStaffWorkspace(staff) {
  ...
  return {
    ...
    notifications: buildStaffNotifications(
      staff,
      [...mappedHomeworks, ...mappedCorrections, ...mappedAnalysisHomeworks],
      [...mappedStats, ...mappedCorrectionStats, ...mappedAnalysisStats],
    ),
  };
}
...
function buildStaffNotifications(staff, homeworks, stats) {
  const statKey = (item) => `${item?.needsHomeworkReview ? "analysis" : item?.isCorrection ? "correction" : "homework"}:${Number(item?.homeworkTemplateId || 0)}`;
  ...
  if (staff.role === "Teacher") {
    return homeworks
      ...
      .map((homework) => {
        ...
        return {
          notificationId: `${needsHomeworkReview ? "teacher-review-needed" : isCorrection ? "teacher-correction" : "teacher-submission"}-${homework.homeworkSubmissionId}`,
  ...
  return homeworks
    ...
    .map((homework) => {
      ...
      const needsHomeworkReview = Boolean(homework.needsHomeworkReview);
      return {
        notificationId: `${needsHomeworkReview ? "curator-review-needed" : isCorrection ? "curator-correction" : "curator-checked"}-${homework.homeworkSubmissionId}`,
        type: needsHomeworkReview ? "HomeworkReviewRequired" : isCorrection ? "CorrectionChecked" : "HomeworkChecked",
