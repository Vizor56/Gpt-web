...
function mapCorrectionAttempts(value) {
  const attempts = Array.isArray(value) ? value : [];
  return attempts.map((attempt) => ({
    ...attempt,
    attemptNumber: Number(attempt.attemptNumber || attempt.attempt_number || 1),
    submittedAt: toDateText(attempt.submittedAt),
    checkedAt: toDateText(attempt.checkedAt),
  }));
}
...
async function runRuntimeMigrations() {
  await dbQuery(`
    ...
    ALTER TABLE lessons ADD COLUMN IF NOT EXISTS homework_review_homework_url TEXT;
    ...
    DO $$
    BEGIN
      ALTER TABLE shop_items DROP CONSTRAINT IF EXISTS shop_items_audience_check;
      ALTER TABLE shop_items
        ADD CONSTRAINT shop_items_audience_check
        CHECK (audience IN ('All', 'Teacher', 'Curator', 'Admin'));
    END $$;
    ...
    ALTER TABLE homework_corrections ADD COLUMN IF NOT EXISTS attempt_number INTEGER NOT NULL DEFAULT 1;

    DO $$
    BEGIN
      ALTER TABLE homework_corrections DROP CONSTRAINT IF EXISTS homework_corrections_assignment_id_key;
      ALTER TABLE homework_corrections DROP CONSTRAINT IF EXISTS homework_corrections_attempt_number_check;
      ALTER TABLE homework_corrections
        ADD CONSTRAINT homework_corrections_attempt_number_check
        CHECK (attempt_number IN (1, 2));
    END $$;

    CREATE TABLE IF NOT EXISTS homework_review_assignments (
      id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      homework_assignment_id INTEGER NOT NULL REFERENCES homework_assignments(id) ON DELETE CASCADE,
    ...
    ALTER TABLE point_transactions ADD COLUMN IF NOT EXISTS homework_review_assignment_id INTEGER;
    ...
    BEGIN
      ...
      ALTER TABLE staff_resource_reviews
        ...
        CHECK (resource_type IN ('Lesson', 'Note', 'Homework', 'Stream', 'Correction', 'HomeworkReview', 'HomeworkReviewAssignment'));
    END $$;
    ...
    BEGIN
      ...
      ALTER TABLE chats
        ...
        CHECK (chat_type IN ('Legacy', 'StudentTeacher', 'StudentCurator', 'StudentAdmin', 'TeacherAdmin', 'CuratorAdmin', 'StudentFriend'));
    END $$;
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_corrections_assignment_attempt ON homework_corrections(assignment_id, attempt_number);
    CREATE INDEX IF NOT EXISTS idx_corrections_assignment_status ON homework_corrections(assignment_id, attempt_number, status);
    CREATE INDEX IF NOT EXISTS idx_homework_review_assignments_student ON homework_review_assignments(student_id, status);
    CREATE INDEX IF NOT EXISTS idx_homework_review_assignments_teacher ON homework_review_assignments(teacher_id, status);
...
async function seedDatabase() {
  ...
  try {
    ...
    await client.query(
      `
        INSERT INTO staff_point_transactions (staff_role, staff_id, points, reason)
        SELECT role, staff_id, 900, 'Стартовый баланс'
        FROM (
          SELECT 'Teacher'::text AS role, id AS staff_id FROM teachers WHERE id <= 4
          UNION ALL
          SELECT 'Curator'::text AS role, id AS staff_id FROM curators WHERE id <= 3
          UNION ALL
          SELECT 'Admin'::text AS role, id AS staff_id FROM admin_accounts WHERE id <= 4
        ) staff_seed
        WHERE NOT EXISTS (
          SELECT 1
          FROM staff_point_transactions existing
          WHERE existing.staff_role = staff_seed.role
            AND existing.staff_id = staff_seed.staff_id
            AND existing.reason = 'Стартовый баланс'
        )
      `,
    );
    ...
    const shopItems = [
      ...
      [25, "pet_dog", "Пиксельный пёс", "animal", 420, "dog", "Верный напарник для марафонов по вариантам."],
      [26, "pet_bunny", "Пиксельный кролик", "animal", 460, "bunny", "Быстрый кролик для тех, кто закрывает дедлайны заранее."],
      [27, "pet_penguin", "Пиксельный пингвин", "animal", 520, "penguin", "Спокойный пингвин для холодной головы на экзамене."],
      [28, "pet_frog", "Пиксельная лягушка", "animal", 390, "frog", "Мемная, но очень собранная лягушка."],
      [29, "fur_midnight", "Ночная шерсть", "fur", 230, "fur-midnight", "Темная шерсть с аккуратным пиксельным контрастом."],
      [30, "fur_rose", "Розовая шерсть", "fur", 240, "fur-rose", "Мягкий цвет для доброго, но уверенного вайба."],
      [31, "fur_gold", "Золотая шерсть", "fur", 320, "fur-gold", "Для тех, кто уверенно набирает баллы."],
      [32, "eyes_neon", "Неоновые глаза", "eyes", 260, "eyes-neon", "Сияющий взгляд в стиле аркадной игры."],
      [33, "scene_beach", "Пляж после экзамена", "scene", 340, "scene-beach", "Песок, небо и ощущение: всё сдано."],
      [34, "scene_classroom", "Пиксельный класс", "scene", 280, "scene-classroom", "Уютная учебная комната с доской."],
      [35, "scene_stadium", "Волейбольная площадка", "scene", 360, "scene-stadium", "Фон для спортивного режима и командного духа."],
      [36, "anime_shades", "Очки главного героя", "face", 370, "anime-shades", "Когда решение пришло на последней минуте."],
      [37, "ninja_band", "Повязка фокуса", "head", 330, "ninja-band", "Для тихой концентрации и резкого рывка."],
      [38, "volleyball", "Волейбольный мяч", "hand", 310, "volleyball", "Мяч для победного розыгрыша после домашки."],
      [39, "skateboard", "Скейт", "hand", 430, "skateboard", "Катиться к дедлайну красиво."],
      [40, "tracksuit", "Спортивный костюм", "outfit", 410, "tracksuit", "Для режима: сделал ДЗ, сделал разминку."],
      [41, "anim_dance", "Анимация: танец", "animation", 500, "dance", "Питомец танцует после продуктивной учебной сессии."],
      ...
      [47, "theme_academy_paper", "Академический лист", "theme", 1500, "theme-academy-paper", "Теплая премиальная тема в стиле чистого авторского конспекта."],
      [48, "theme_pixel_twilight", "Пиксельный вечер", "theme", 1700, "theme-pixel-twilight", "Глубокая вечерняя тема с мягкими пиксельными акцентами и спокойной навигацией."],
      [49, "theme_formula_lab", "Лаборатория формул", "theme", 1900, "theme-formula-lab", "Технологичная тема с сетками, бирюзой и ощущением кабинета физики."],
      [50, "theme_ocean_focus", "Тихий океан", "theme", 2100, "theme-ocean-focus", "Глубокая спокойная тема с холодной водой, ясными панелями и мягким золотым акцентом."],
      [51, "theme_sunrise_notebook", "Утренний конспект", "theme", 2300, "theme-sunrise-notebook", "Теплая тема с ощущением раннего занятия, чистой тетради и аккуратных учебных пометок."],
      [52, "theme_pixel_cats", "Пиксельные коты", "theme", 2600, "theme-pixel-cats", "Спокойная брендовая тема с мягкими пиксельными котами на фоне."],
      [53, "theme_pixel_dogs", "Пиксельные собаки", "theme", 2600, "theme-pixel-dogs", "Теплая тема с аккуратными пиксельными собачками и рабочей палитрой Yaroslav Tutor."],
      [54, "theme_pixel_hearts", "Пиксельные сердечки", "theme", 2400, "theme-pixel-hearts", "Светлая тема с маленькими пиксельными сердцами и спокойными учебными акцентами."],
      [55, "theme_pixel_coffee", "Пиксельный кофе", "theme", 2800, "theme-pixel-coffee", "Собранная кофейная тема для долгой учебы, заметок и вечерних разборов."],
      [67, "theme_quiet_geometry", "Тихая геометрия", "theme", 3000, "theme-quiet-geometry", "Чистая математическая тема с тонкой сеткой, точками и спокойной бирюзовой логикой."],
      [68, "theme_physics_lab", "Физическая лаборатория", "theme", 3200, "theme-physics-lab", "Собранная тема с координатами, траекториями и стальным акцентом для физики."],
      [69, "theme_coffee_notes", "Кофейный конспект", "theme", 3400, "theme-coffee-notes", "Уютная тема для вечерней учебы: молочный фон, карамель и аккуратные листы заметок."],
      [70, "theme_northern_lights", "Северное сияние", "theme", 3600, "theme-northern-lights", "Глубокая спокойная тема с холодной мятой, графитом и тонкими световыми линиями."],
      [71, "theme_solution_garden", "Сад решений", "theme", 3300, "theme-solution-garden", "Мягкая зеленая тема про рост, маршрут решения и спокойный прогресс."],
      [72, "theme_pixel_academy", "Пиксельная академия", "theme", 3800, "theme-pixel-academy", "Учебная RPG без детскости: пиксельные маркеры прогресса в премиальной палитре."],
      [56, "library_book_math", "Книга: Математика без паники", "library_book", 260, "library-book-math", "Бирюзовая книга для библиотеки конспектов."],
      [57, "library_book_physics", "Книга: Физика на пальцах", "library_book", 280, "library-book-physics", "Синяя книга для спокойного разбора законов."],
      [58, "library_book_comics", "Книга: Мемный задачник", "library_book", 320, "library-book-comics", "Веселая книга, когда хочется учиться без лишнего напряжения."],
      [59, "library_book_notes", "Книга: Тетрадь инсайтов", "library_book", 340, "library-book-notes", "Аккуратный конспект с личными открытиями."],
      [60, "library_place_park", "Читать в парке", "library_place", 360, "library-place-park", "Лавка, зелень и спокойное повторение."],
      [61, "library_place_science", "Научная библиотека", "library_place", 420, "library-place-science", "Полки, тишина и ощущение большой системы знаний."],
      [62, "library_place_garden", "Сад у дома", "library_place", 380, "library-place-garden", "Теплое место для вечерних конспектов."],
      [63, "library_place_river", "У реки", "library_place", 410, "library-place-river", "Свежий воздух и спокойный ритм задач."],
    ...
    const staffShopItems = [
      [101, "teacher_pet_owl", "Сова наставника", "animal", 360, "teacher-owl", "Спокойный питомец для продуманных объяснений.", "Teacher"],
      [102, "teacher_pet_lion", "Лев лекций", "animal", 430, "teacher-lion", "Уверенный образ для сильных разборов.", "Teacher"],
      [103, "teacher_fur_graphite", "Графитовая шерсть", "fur", 230, "fur-teacher-graphite", "Сдержанный цвет для рабочего профиля.", "Teacher"],
      [104, "teacher_eyes_amber", "Янтарный взгляд", "eyes", 210, "eyes-teacher-amber", "Теплый взгляд, который держит внимание на уроке.", "Teacher"],
      [105, "teacher_scene_board", "Аудитория с доской", "scene", 330, "scene-teacher-board", "Доска, формулы и чистый фон для преподавателя.", "Teacher"],
      [106, "teacher_scene_library", "Библиотека курса", "scene", 340, "scene-teacher-library", "Полки, книги и атмосфера готового конспекта.", "Teacher"],
      [107, "teacher_anim_point", "Анимация: указка", "animation", 390, "point", "Питомец делает акцент, как на важном моменте урока.", "Teacher"],
      [108, "teacher_anim_grade", "Анимация: оценка", "animation", 420, "grade", "Короткий жест после проверки работы.", "Teacher"],
      [109, "teacher_blazer", "Преподавательский пиджак", "outfit", 360, "teacher-blazer", "Аккуратный пиджак для видеоуроков.", "Teacher"],
      [110, "teacher_cardigan", "Кардиган наставника", "outfit", 340, "teacher-cardigan", "Мягкий образ для долгих разборов.", "Teacher"],
      [111, "teacher_pointer", "Указка", "hand", 240, "teacher-pointer", "Тонкая указка для формул и схем.", "Teacher"],
      [112, "teacher_book", "Книга методик", "hand", 260, "teacher-book", "Аккуратная книга с пометками для уроков.", "Teacher"],
      [121, "curator_pet_deer", "Олень маршрута", "animal", 360, "curator-deer", "Питомец, который аккуратно ведет учеников по плану.", "Curator"],
      [122, "curator_pet_otter", "Выдра связи", "animal", 390, "curator-otter", "Дружелюбный питомец для созвонов и чатов.", "Curator"],
      [123, "curator_fur_mint", "Мятная шерсть", "fur", 230, "fur-curator-mint", "Свежий цвет для кураторского кабинета.", "Curator"],
      [124, "curator_eyes_violet", "Фиолетовый взгляд", "eyes", 210, "eyes-curator-violet", "Мягкий акцент для обратной связи.", "Curator"],
      [125, "curator_scene_lounge", "Кабинет созвонов", "scene", 320, "scene-curator-lounge", "Спокойный фон для обсуждения прогресса.", "Curator"],
      [126, "curator_scene_calendar", "Планер группы", "scene", 330, "scene-curator-calendar", "Календарь, заметки и видимый ритм учебы.", "Curator"],
      [127, "curator_anim_checklist", "Анимация: чек-лист", "animation", 390, "checklist", "Питомец отмечает выполненные шаги.", "Curator"],
      [128, "curator_anim_call", "Анимация: созвон", "animation", 410, "call", "Живой акцент для кураторских встреч.", "Curator"],
      [129, "curator_vest", "Жилет куратора", "outfit", 340, "curator-vest", "Удобный образ для ежедневной связи.", "Curator"],
      [130, "curator_soft_hoodie", "Худи поддержки", "outfit", 350, "curator-hoodie", "Неформально, но очень аккуратно.", "Curator"],
      [131, "curator_headset", "Гарнитура куратора", "face", 240, "curator-headset", "Для быстрых ответов в чатах и на созвонах.", "Curator"],
      [132, "curator_planner", "Планер", "hand", 260, "curator-planner", "Планер с датами, который всегда под рукой.", "Curator"],
      [141, "admin_pet_robot", "Робот деплоя", "animal", 380, "admin-robot", "Питомец для стабильных релизов и логов.", "Admin"],
      [142, "admin_pet_raven", "Ворон наблюдения", "animal", 420, "admin-raven", "Строгий питомец для панели администратора.", "Admin"],
      [143, "admin_fur_steel", "Стальная шерсть", "fur", 240, "fur-admin-steel", "Чистый технический оттенок для админа.", "Admin"],
      [144, "admin_eyes_cyan", "Циановый взгляд", "eyes", 220, "eyes-admin-cyan", "Подсветка для мониторинга данных.", "Admin"],
      [145, "admin_scene_server", "Серверная", "scene", 340, "scene-admin-server", "Узнаваемый фон с индикаторами и статусом.", "Admin"],
      [146, "admin_scene_dashboard", "Панель данных", "scene", 350, "scene-admin-dashboard", "Графики, заявки и все важное перед глазами.", "Admin"],
      [147, "admin_anim_deploy", "Анимация: деплой", "animation", 420, "deploy", "Питомец делает мини-запуск после обновления.", "Admin"],
      [148, "admin_anim_scan", "Анимация: скан", "animation", 410, "scan", "Быстрая проверка состояния системы.", "Admin"],
      [149, "admin_jacket", "Админская куртка", "outfit", 360, "admin-jacket", "Темная куртка с аккуратным интерфейсным акцентом.", "Admin"],
      [150, "admin_suit", "Костюм системы", "outfit", 380, "admin-suit", "Собранный вид для главной панели.", "Admin"],
      [151, "admin_laptop", "Ноутбук", "hand", 280, "admin-laptop", "Мини-ноутбук для заявок, базы и чатов.", "Admin"],
      [152, "admin_keycard", "Ключ-карта", "neck", 260, "admin-keycard", "Пропуск в админскую версию сайта.", "Admin"],
    ];

    for (const item of [...shopItems, ...staffShopItems]) {
      const [id, itemCode, itemName, itemType, pricePoints, cssClass, description, audience = "All"] = item;
      await client.query(
        `
          INSERT INTO shop_items (id, item_code, item_name, item_type, price_points, css_class, description, audience, active)
          ...
          ON CONFLICT (item_code) DO UPDATE SET
            ...
            active = TRUE
        `,
        [id, itemCode, itemName, itemType, pricePoints, cssClass, description, audience],
      );
    }
    ...
    const profileBadges = [
      ["shop_lofi", "Lo-fi режим", "shop", 260, "badge-lofi", "Тихая плашка для спокойной учебы.", null],
      ["shop_main_char", "Главный герой", "shop", 420, "badge-main", "Когда сегодня явно твоя серия.", null],
      ["shop_sigma", "Sigma focus", "shop", 390, "badge-sigma", "Без лишнего шума, только результат.", null],
      ["shop_waifu_math", "Math arc", "shop", 360, "badge-math-arc", "Для тех, кто пережил арку параметров.", null],
      ["shop_meme_lord", "Meme lord", "shop", 440, "badge-meme", "Мемно, но домашка сдана.", null],
      ["shop_exam_slayer", "Exam slayer", "shop", 520, "badge-slayer", "Плашка для охотника за баллами.", null],
      ["shop_pixel_aura", "Pixel aura", "shop", 480, "badge-aura", "Светится как правильное решение.", null],
      ["shop_chill_boss", "Chill boss", "shop", 400, "badge-chill", "Спокойствие, дедлайн под контролем.", null],
      ["shop_speedrun", "Speedrun", "shop", 460, "badge-speedrun", "Для быстрых и аккуратных.", null],
      ["shop_final_form", "Final form", "shop", 680, "badge-final", "Когда сборка почти идеальная.", null],
      ["ach_start", "Первый шаг", "achievement", 0, "badge-ach-start", "Открывается за 5% прогресса курса.", 5],
      ["ach_10", "Разогрев", "achievement", 0, "badge-ach-10", "Открывается за 10% прогресса курса.", 10],
      ["ach_15", "Стабильный темп", "achievement", 0, "badge-ach-15", "Открывается за 15% прогресса курса.", 15],
      ["ach_20", "Вошел в ритм", "achievement", 0, "badge-ach-20", "Открывается за 20% прогресса курса.", 20],
      ["ach_25", "Четверть пути", "achievement", 0, "badge-ach-25", "Открывается за 25% прогресса курса.", 25],
      ["ach_30", "Комбо x3", "achievement", 0, "badge-ach-30", "Открывается за 30% прогресса курса.", 30],
      ["ach_35", "Фокусник", "achievement", 0, "badge-ach-35", "Открывается за 35% прогресса курса.", 35],
      ["ach_40", "Почти середина", "achievement", 0, "badge-ach-40", "Открывается за 40% прогресса курса.", 40],
      ["ach_45", "Уверенный ход", "achievement", 0, "badge-ach-45", "Открывается за 45% прогресса курса.", 45],
      ["ach_50", "Экватор", "achievement", 0, "badge-ach-50", "Открывается за 50% прогресса курса.", 50],
      ["ach_55", "После экватора", "achievement", 0, "badge-ach-55", "Открывается за 55% прогресса курса.", 55],
      ["ach_60", "Шестьдесят", "achievement", 0, "badge-ach-60", "Открывается за 60% прогресса курса.", 60],
      ["ach_65", "Разгон", "achievement", 0, "badge-ach-65", "Открывается за 65% прогресса курса.", 65],
      ["ach_70", "Серьезная заявка", "achievement", 0, "badge-ach-70", "Открывается за 70% прогресса курса.", 70],
      ["ach_75", "Три четверти", "achievement", 0, "badge-ach-75", "Открывается за 75% прогресса курса.", 75],
      ["ach_80", "Финиш близко", "achievement", 0, "badge-ach-80", "Открывается за 80% прогресса курса.", 80],
      ["ach_85", "Почти ульта", "achievement", 0, "badge-ach-85", "Открывается за 85% прогресса курса.", 85],
      ["ach_90", "Девяносто", "achievement", 0, "badge-ach-90", "Открывается за 90% прогресса курса.", 90],
      ["ach_95", "Финальный рывок", "achievement", 0, "badge-ach-95", "Открывается за 95% прогресса курса.", 95],
      ["ach_100", "Курс закрыт", "achievement", 0, "badge-ach-100", "Открывается за 100% прогресса курса.", 100],
    ];

    for (const badge of profileBadges) {
      await client.query(
        `
          INSERT INTO profile_badges (badge_code, badge_name, badge_type, price_points, css_class, description, unlock_percent, active)
...
async function insertHomeworkPoints(client, assignmentId, studentId) {
  await client.query(
    `
      ...
      SELECT ha.student_id, ha.enrollment_id, ha.id, 60, 'Баллы за сданное ДЗ'
      FROM homework_assignments ha
      WHERE ha.id = $1
        AND ha.student_id = $2
        AND NOT EXISTS (
...
async function insertCorrectionPoints(client, correctionId, studentId) {
  await client.query(
    `
...
async function insertReviewHomeworkPoints(client, reviewAssignmentId, studentId) {
  await client.query(
    `
      INSERT INTO point_transactions (student_id, enrollment_id, homework_assignment_id, homework_review_assignment_id, points, reason)
      SELECT hra.student_id, hra.enrollment_id, hra.homework_assignment_id, hra.id, 35, 'Баллы за ДЗ V.2.0'
      FROM homework_review_assignments hra
      WHERE hra.id = $1
        AND hra.student_id = $2
        AND NOT EXISTS (
          SELECT 1
          FROM point_transactions pt
          WHERE pt.homework_review_assignment_id = hra.id
            AND pt.student_id = hra.student_id
            AND pt.reason = 'Баллы за ДЗ V.2.0'
        )
    `,
    [reviewAssignmentId, studentId],
  );
}
...
function correctionPassedExistsSql(assignmentAlias = "ha") {
  return `EXISTS (
    SELECT 1
    FROM homework_corrections hc_pass
    WHERE hc_pass.assignment_id = ${assignmentAlias}.id
      AND hc_pass.status = 'Checked'
      AND ${scoreSql("hc_pass")} >= 5
  )`;
}

function homeworkNeedsReviewSql(assignmentAlias = "ha") {
  return `EXISTS (
    SELECT 1
...
function reviewHomeworkCheckedExistsSql(assignmentAlias = "ha") {
  return `EXISTS (
    SELECT 1
    FROM homework_review_assignments hra_checked
    WHERE hra_checked.homework_assignment_id = ${assignmentAlias}.id
      AND hra_checked.status = 'Checked'
  )`;
}

function reviewHomeworkLowCheckedExistsSql(assignmentAlias = "ha") {
  return `EXISTS (
    SELECT 1
    FROM homework_review_assignments hra_low
    WHERE hra_low.homework_assignment_id = ${assignmentAlias}.id
      AND hra_low.status = 'Checked'
      AND COALESCE(${scoreSql("hra_low")}, 0) < 5
  )`;
}

function reviewHomeworkSubmittedExistsSql(assignmentAlias = "ha") {
  return `EXISTS (
    SELECT 1
    FROM homework_review_assignments hra_submitted
    WHERE hra_submitted.homework_assignment_id = ${assignmentAlias}.id
      AND hra_submitted.status = 'Submitted'
  )`;
}

function lessonPassedSql(assignmentAlias = "ha", submissionAlias = "hs") {
  return `(COALESCE(${passedCheckedSql(submissionAlias)}, FALSE) OR ${correctionPassedExistsSql(assignmentAlias)} OR ${reviewHomeworkCheckedExistsSql(assignmentAlias)})`;
}

function activeCorrectionJoinSql(assignmentAlias = "ha", alias = "hc") {
  return `
      LEFT JOIN LATERAL (
        SELECT *
        FROM homework_corrections ${alias}_pick
        WHERE ${alias}_pick.assignment_id = ${assignmentAlias}.id
          AND ${alias}_pick.status <> 'Cancelled'
        ORDER BY
          CASE WHEN ${alias}_pick.status IN ('Required', 'Submitted') THEN 0 ELSE 1 END,
          ${alias}_pick.attempt_number DESC,
          ${alias}_pick.id DESC
        LIMIT 1
      ) ${alias} ON TRUE
  `;
...
function activeReviewHomeworkJoinSql(assignmentAlias = "ha", alias = "hra") {
  return `
      LEFT JOIN LATERAL (
        SELECT *
        FROM homework_review_assignments ${alias}_pick
        WHERE ${alias}_pick.homework_assignment_id = ${assignmentAlias}.id
          AND ${alias}_pick.status <> 'Cancelled'
        ORDER BY
          CASE WHEN ${alias}_pick.status IN ('Assigned', 'Submitted') THEN 0 ELSE 1 END,
          ${alias}_pick.id DESC
        LIMIT 1
      ) ${alias} ON TRUE
  `;
...
function correctionAttemptsJsonSql(assignmentAlias = "ha") {
  return `COALESCE((
    SELECT JSON_AGG(
      JSON_BUILD_OBJECT(
        'correctionId', hca.id,
        'attemptNumber', hca.attempt_number,
        'status', hca.status,
        'submittedUrl', hca.file_url,
        'submittedAt', hca.submitted_at,
        'score', hca.score,
        'feedbackText', hca.feedback_text,
        'checkedAt', hca.checked_at
      )
      ORDER BY hca.attempt_number
    )
    FROM homework_corrections hca
    WHERE hca.assignment_id = ${assignmentAlias}.id
...
function lessonLearningStatusSql(lessonAlias = "l", assignmentAlias = "ha", submissionAlias = "hs", correctionAlias = "hc") {
  ...
  return `
    CASE
      WHEN ${reviewHomeworkLowCheckedExistsSql(assignmentAlias)} THEN 'Review_V2_Focus'
      WHEN ${lessonPassedSql(assignmentAlias, submissionAlias)} THEN 'Done'
      WHEN ${reviewHomeworkSubmittedExistsSql(assignmentAlias)} THEN 'Review_V2_Check'
      WHEN ${homeworkNeedsReviewSql(assignmentAlias)}
        AND COALESCE(${lessonAlias}.homework_review_url, '') = ''
        THEN 'Review_Pending'
      WHEN ${homeworkNeedsReviewSql(assignmentAlias)}
        AND COALESCE(${lessonAlias}.homework_review_url, '') <> ''
        AND COALESCE(${lessonAlias}.homework_review_homework_url, '') <> ''
        THEN 'Review_V2_Required'
      WHEN ${homeworkNeedsReviewSql(assignmentAlias)} THEN 'Review_Required'
      WHEN ${correctionAlias}.status = 'Submitted' AND ${correctionAlias}.attempt_number = 2 THEN 'Correction2_Check'
      WHEN ${correctionAlias}.status = 'Submitted' THEN 'Correction_Check'
      WHEN ${correctionAlias}.id IS NOT NULL
        AND ${correctionAlias}.attempt_number = 2
        AND (
          ${correctionAlias}.status = 'Required'
          OR (${correctionAlias}.status = 'Checked' AND COALESCE(${correctionScore}, 0) < 5)
        )
        THEN 'Correction2_Required'
      WHEN ${correctionAlias}.id IS NOT NULL
        AND (
          ${correctionAlias}.status = 'Required'
          OR (${correctionAlias}.status = 'Checked' AND COALESCE(${correctionScore}, 0) < 5)
      ...
      SET progress_percent = COALESCE(
        (
          SELECT
            CASE
              WHEN COUNT(*) = 0 THEN 0
              ELSE ROUND(100.0 * SUM(CASE WHEN ${lessonPassedSql("ha", "hs")} THEN 1 ELSE 0 END) / COUNT(*), 2)
            END
          FROM homework_assignments ha
...
async function ensureReviewHomeworkAssignmentsForLesson(client, lessonId, taskUrl = "") {
  if (!lessonId || !taskUrl) {
    return;
  }
  ...
  await client.query(
    `
      INSERT INTO homework_review_assignments (homework_assignment_id, lesson_id, student_id, teacher_id, enrollment_id, task_url, status)
      SELECT
        ha.id,
        l.id,
        ha.student_id,
        COALESCE(ha.teacher_id, ce.teacher_id, c.teacher_id),
        ha.enrollment_id,
        COALESCE(NULLIF(l.homework_review_homework_url, ''), $2),
        'Assigned'
      FROM lessons l
      JOIN homework_templates ht ON ht.lesson_id = l.id AND ht.course_id = l.course_id AND ht.active = TRUE
      JOIN homework_assignments ha ON ha.template_id = ht.id AND ha.status <> 'Cancelled'
      JOIN courses c ON c.id = l.course_id
      LEFT JOIN course_enrollments ce ON ce.id = ha.enrollment_id
      WHERE l.id = $1
        AND COALESCE(l.homework_review_url, '') <> ''
        AND COALESCE(NULLIF(l.homework_review_homework_url, ''), $2, '') <> ''
        AND ${homeworkNeedsReviewSql("ha")}
      ON CONFLICT (homework_assignment_id, student_id) DO UPDATE SET
        ...
        status = CASE
          WHEN homework_review_assignments.status = 'Cancelled' THEN 'Assigned'
          ELSE homework_review_assignments.status
        END,
        updated_at = NOW()
    `,
    [lessonId, taskUrl],
...
async function buildAccountPayload(studentId, authToken = "") {
  ...
  const courseRows = await dbQuery(
    `
      SELECT
        ...
        COUNT(DISTINCT CASE WHEN ${lessonPassedSql("ha", "hs")} THEN l.id END)::integer AS "lessonsCompleted",
  ...
  const lessonRows = await dbQuery(
    `
      SELECT
        ...
        l.homework_review_homework_url AS "homeworkReviewHomeworkUrl",
        ...
        hra.id AS "reviewHomeworkId",
        hra.status AS "reviewHomeworkStatus",
        COALESCE(hra.task_url, l.homework_review_homework_url) AS "reviewHomeworkTaskUrl",
        hra.file_url AS "reviewHomeworkSubmittedUrl",
        hra.submitted_at AS "reviewHomeworkSubmittedAt",
        hra.score AS "reviewHomeworkScore",
        hra.feedback_text AS "reviewHomeworkFeedbackText",
        hra.checked_at AS "reviewHomeworkCheckedAt"
      FROM course_enrollments ce
      JOIN courses c ON c.id = ce.course_id
      ...
      ${activeReviewHomeworkJoinSql("ha", "hra")}
      WHERE ce.student_id = $1
        AND ce.status = 'Active'
        AND c.status = 'Active'
  ...
  for (const row of lessonRows.rows) {
    ...
    lessonGroups.get(row.courseSlug).push({
      ...
      homeworkReviewHomeworkUrl: row.homeworkReviewHomeworkUrl,
      lessonStatus: row.lessonStatus,
      watchPercent: Number(row.watchPercent || 0),
      isCompleted: Boolean(row.isCompleted),
      completedAt: toDateText(row.completedAt),
      reviewHomeworkId: row.reviewHomeworkId,
      reviewHomeworkStatus: row.reviewHomeworkStatus,
      reviewHomeworkTaskUrl: row.reviewHomeworkTaskUrl,
      reviewHomeworkSubmittedUrl: row.reviewHomeworkSubmittedUrl,
      reviewHomeworkSubmittedAt: toDateText(row.reviewHomeworkSubmittedAt),
      reviewHomeworkScore: row.reviewHomeworkScore,
      reviewHomeworkFeedbackText: row.reviewHomeworkFeedbackText,
      reviewHomeworkCheckedAt: toDateText(row.reviewHomeworkCheckedAt),
    });
  }
    ...
    `
      ...
      FROM (
        ...
        SELECT
          ...
          CASE WHEN hc.score >= 5 THEN 'success' ELSE 'warning' END AS tone
        FROM homework_corrections hc
        ...
        SELECT
          CONCAT('review-homework-', hra.id) AS "notificationId",
          CASE
            WHEN COALESCE(${scoreSql("hra")}, 0) >= 5 THEN 'ДЗ V.2.0 проверено'
            ELSE 'Уделить особое внимание теме'
          END AS title,
          CONCAT(
            c.title,
            ' · Урок ',
            ...
            '. ДЗ V.2.0 · ',
            CASE
              WHEN hra.score IS NULL THEN 'оценка не указана'
              WHEN hra.score > 10 AND hra.score <= 100 THEN ROUND(hra.score::numeric / 10, 1)::text
              WHEN hra.score > 100 THEN '10'
              ELSE hra.score::text
            END,
            '/10 баллов'
          ) AS text,
          'Homework_Review_Assignment' AS type,
          CONCAT('#course-', c.slug) AS link,
          hra.checked_at AS "createdAt",
          CASE WHEN COALESCE(${scoreSql("hra")}, 0) >= 5 THEN 'success' ELSE 'warning' END AS tone
        FROM homework_review_assignments hra
        JOIN homework_assignments ha ON ha.id = hra.homework_assignment_id
        JOIN homework_templates ht ON ht.id = ha.template_id
        JOIN courses c ON c.id = ht.course_id
        LEFT JOIN lessons l ON l.id = COALESCE(hra.lesson_id, ht.lesson_id)
        WHERE hra.student_id = $1
          AND hra.status = 'Checked'
          AND hra.checked_at IS NOT NULL
...
async function getCourseLessons(slug, studentId = 0) {
  const rows = await dbQuery(
    `
      SELECT
        ...
        hc.attempt_number AS "correctionAttempt",
        ...
        hra.feedback_text AS "reviewHomeworkFeedbackText",
        hra.checked_at AS "reviewHomeworkCheckedAt",
        ${homeworkNeedsReviewSql("ha")} AS "needsHomeworkReview",
        ${correctionAttemptsJsonSql("ha")} AS "correctionAttempts"
      FROM courses c
      JOIN lessons l ON l.course_id = c.id
  ...
  const rawLessons = rows.rows.map((row) => ({
    ...
    correctionAttempt: row.correctionAttempt,
    correctionStatus: row.correctionStatus,
    correctionSubmittedUrl: row.correctionSubmittedUrl,
    correctionSubmittedAt: toDateText(row.correctionSubmittedAt),
    correctionScore: row.correctionScore,
    correctionFeedbackText: row.correctionFeedbackText,
    ...
    reviewHomeworkFeedbackText: row.reviewHomeworkFeedbackText,
    reviewHomeworkCheckedAt: toDateText(row.reviewHomeworkCheckedAt),
    needsHomeworkReview: Boolean(row.needsHomeworkReview),
    correctionAttempts: mapCorrectionAttempts(row.correctionAttempts),
  }));
  const lessons = isOwned
    ...
    : rawLessons.map((lesson, index) => ({
        ...
        homeworkReviewHomeworkUrl: index === 0 ? lesson.homeworkReviewHomeworkUrl : null,
        ...
        reviewHomeworkId: null,
        reviewHomeworkStatus: null,
        reviewHomeworkTaskUrl: null,
        reviewHomeworkSubmittedUrl: null,
        reviewHomeworkSubmittedAt: null,
        reviewHomeworkScore: null,
        reviewHomeworkFeedbackText: null,
        reviewHomeworkCheckedAt: null,
        needsHomeworkReview: false,
        correctionAttempts: [],
        isCompleted: false,
      }));
...
app.post(
  "/api/homeworks/:id/correction/submit",
  asyncRoute(async (request, response) => {
    const student = await getStudentByToken(request);
    const assignmentId = toInt(request.params.id, 0);
    ...
    try {
      ...
      const correction = await client.query(
        `
          ...
          ORDER BY
            CASE WHEN hc.status IN ('Required', 'Submitted') THEN 0 ELSE 1 END,
            hc.attempt_number DESC,
            hc.id DESC
          LIMIT 1
          FOR UPDATE
        `,
      ...
      if (row.status === "Checked" && Number(row.score || 0) < 5 && Number(row.attempt_number || 1) < 2) {
        const nextAttempt = Number(row.attempt_number || 1) + 1;
        const nextCorrection = await client.query(
          `
            INSERT INTO homework_corrections (assignment_id, attempt_number, student_id, teacher_id, enrollment_id, required_score, status)
            VALUES ($1, $2, $3, $4, $5, $6, 'Required')
            ON CONFLICT (assignment_id, attempt_number) DO UPDATE SET
              teacher_id = EXCLUDED.teacher_id,
              enrollment_id = EXCLUDED.enrollment_id,
              required_score = EXCLUDED.required_score,
              status = CASE
                WHEN homework_corrections.status = 'Cancelled' THEN 'Required'
                ELSE homework_corrections.status
              END,
              updated_at = NOW()
            RETURNING id, assignment_id, student_id, enrollment_id, teacher_id, attempt_number, status, score
          `,
          [assignmentId, nextAttempt, row.student_id, row.teacher_id, row.enrollment_id, row.score],
        );
        row = nextCorrection.rows[0];
      }
      ...
      const updated = await client.query(
        `
          ...
          RETURNING id, submitted_at
        `,
        [row.id, correctionLink],
      );

      await insertCorrectionPoints(client, row.id, student.studentId);
      if (row.enrollment_id) {
        await refreshEnrollmentProgress(client, row.enrollment_id);
      }
      await client.query("COMMIT");
      ...
      response.json({
        ...
        homeworkAssignmentId: assignmentId,
        correctionId: row.id,
        correctionAttempt: row.attempt_number,
        correctionStatus: "Submitted",
        correctionSubmittedUrl: correctionLink,
        correctionSubmittedAt: toDateText(updated.rows[0].submitted_at),
      });
    } catch (error) {
      await client.query("ROLLBACK");
      throw error;
...
app.post(
  "/api/homeworks/:id/review-homework/submit",
  asyncRoute(async (request, response) => {
    const student = await getStudentByToken(request);
    const assignmentId = toInt(request.params.id, 0);
    const reviewHomeworkLink = normalizeLink(request.body.reviewHomeworkLink || request.body.homeworkLink);

    if (!assignmentId) {
      throw createHttpError(400, "Некорректный ID домашнего задания.");
    }

    if (!isGoogleDriveLikeUrl(reviewHomeworkLink)) {
      throw createHttpError(400, "Нужна ссылка на Google Drive.");
    }
    ...
    try {
      ...
      const assignment = await client.query(
        `
          SELECT
            ...
            ha.student_id,
            ha.enrollment_id,
            COALESCE(ha.teacher_id, ce.teacher_id, c.teacher_id) AS teacher_id,
            l.id AS lesson_id,
            l.homework_review_url,
            l.homework_review_homework_url,
            ${homeworkNeedsReviewSql("ha")} AS needs_review
          FROM homework_assignments ha
          JOIN homework_templates ht ON ht.id = ha.template_id
          JOIN courses c ON c.id = ht.course_id
          LEFT JOIN lessons l ON l.id = ht.lesson_id
      ...
      const base = assignment.rows[0];
      if (!base.needs_review) {
        throw createHttpError(403, "ДЗ V.2.0 можно сдавать только после двух работ над ошибками ниже 5.");
      }

      if (!base.homework_review_url || !base.homework_review_homework_url) {
        throw createHttpError(400, "Преподаватель ещё не добавил разбор и ДЗ V.2.0 к этому уроку.");
      }

      await ensureReviewHomeworkAssignmentsForLesson(client, base.lesson_id, base.homework_review_homework_url);

      const reviewAssignment = await client.query(
        `
          SELECT *
          FROM homework_review_assignments
          WHERE homework_assignment_id = $1
            AND student_id = $2
      ...
      if (reviewAssignment.rowCount === 0) {
        throw createHttpError(500, "ДЗ V.2.0 не удалось подготовить в базе.");
      }

      const row = reviewAssignment.rows[0];
      if (row.status === "Submitted" || row.status === "Checked" || row.file_url) {
        throw createHttpError(409, "Отправленную ссылку на ДЗ V.2.0 нельзя заменить.");
      }

      const updated = await client.query(
        `
          UPDATE homework_review_assignments
          SET status = 'Submitted',
              student_text = 'Ссылка на ДЗ V.2.0 прикреплена с сайта.',
              file_url = $2,
              submitted_at = NOW(),
              checked_by_teacher_id = NULL,
              checked_at = NULL,
          ...
          RETURNING id, submitted_at
        `,
        [row.id, reviewHomeworkLink],
      );

      await insertReviewHomeworkPoints(client, row.id, student.studentId);
      if (base.enrollment_id) {
        await refreshEnrollmentProgress(client, base.enrollment_id);
      }
      await client.query("COMMIT");
      ...
      response.json({
        ...
        source: "database",
        homeworkAssignmentId: assignmentId,
        reviewHomeworkId: updated.rows[0].id,
        reviewHomeworkStatus: "Submitted",
        reviewHomeworkSubmittedUrl: reviewHomeworkLink,
        reviewHomeworkSubmittedAt: toDateText(updated.rows[0].submitted_at),
      });
    } catch (error) {
      await client.query("ROLLBACK");
      throw error;
...
app.get(
  ...
  asyncRoute(async (request, response) => {
    const student = await getStudentByToken(request);
    const streamId = toInt(request.params.id, 0);

    if (!streamId) {
      throw createHttpError(400, "Трансляция не выбрана.");
    }

    const stream = await dbOne(
      `
        SELECT
          ls.id AS "streamId",
    ...
    if (!stream) {
      throw createHttpError(404, "Трансляция не найдена.");
    }
    ...
    const messages = await dbQuery(
      `
        SELECT
          lsm.id AS "messageId",
          lsm.message_text AS "messageText",
          lsm.created_at AS "createdAt",
          s.id AS "studentId",
        ...
        JOIN students s ON s.id = lsm.student_id
        LEFT JOIN profile_badges pb ON pb.id = s.active_badge_id
        WHERE lsm.stream_id = $1
        ORDER BY lsm.created_at, lsm.id
        LIMIT 300
      `,
      [streamId],
    );
    const studentProfileItems = await getStudentProfileItemsMap(messages.rows.map((row) => row.studentId));

    response.json({
      source: "database",
      stream: { ...stream, startsAt: toDateText(stream.startsAt) },
      currentStudentId: student.studentId,
      messages: messages.rows.map((row) => ({
        ...row,
        createdAt: toDateText(row.createdAt),
        isOwn: Number(row.studentId) === Number(student.studentId),
        profileItems: studentProfileItems.get(Number(row.studentId)) || [],
      })),
    });
  }),
);
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    const student = await getStudentByToken(request);
    const streamId = toInt(request.params.id, 0);
    const messageText = cleanText(request.body.messageText, 600);

    if (!streamId || !messageText) {
      throw createHttpError(400, "Выберите трансляцию и введите сообщение.");
    }

    const stream = await dbOne("SELECT id FROM live_streams WHERE id = $1", [streamId]);
    if (!stream) {
      throw createHttpError(404, "Трансляция не найдена.");
    }

    await dbQuery(
      "INSERT INTO live_stream_messages (stream_id, student_id, message_text) VALUES ($1, $2, $3)",
      [streamId, student.studentId, messageText],
    );

    response.status(201).json({ ok: true, source: "database" });
  }),
);
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    ...
    try {
      await client.query("BEGIN");
      const item = await client.query("SELECT * FROM shop_items WHERE item_code = $1 AND active = TRUE AND audience = 'All'", [itemCode]);
      if (item.rowCount === 0) {
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    ...
    const item = await dbOne("SELECT id, item_type AS \"itemType\" FROM shop_items WHERE item_code = $1 AND active = TRUE AND audience = 'All'", [itemCode]);
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    const student = await getStudentByToken(request);
    const badgeCode = cleanText(request.body.badgeCode, 80);
    const client = await requireDatabase().connect();

    try {
      await client.query("BEGIN");
      const badge = await client.query("SELECT * FROM profile_badges WHERE badge_code = $1 AND active = TRUE AND badge_type = 'shop'", [badgeCode]);
      if (badge.rowCount === 0) {
        throw createHttpError(404, "Плашка не найдена.");
      }

      const owned = await client.query("SELECT 1 FROM student_badges WHERE student_id = $1 AND badge_id = $2", [student.studentId, badge.rows[0].id]);
      if (owned.rowCount > 0) {
        throw createHttpError(409, "Эта плашка уже открыта.");
      }

      const balance = await getStudentPoints(client, student.studentId);
      if (balance < badge.rows[0].price_points) {
        throw createHttpError(400, "Недостаточно баллов.");
      }

      await client.query("INSERT INTO student_badges (student_id, badge_id, source) VALUES ($1, $2, 'shop')", [student.studentId, badge.rows[0].id]);
      await client.query("INSERT INTO point_transactions (student_id, points, reason) VALUES ($1, $2, $3)", [
        student.studentId,
        -Number(badge.rows[0].price_points),
        `Покупка плашки: ${badge.rows[0].badge_name}`,
      ]);
      await client.query("COMMIT");
      response.json(await getShopPayload(student.studentId));
    } catch (error) {
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    ...
    if (unequip) {
      await dbQuery("UPDATE students SET active_badge_id = NULL WHERE id = $1", [student.studentId]);
      response.json(await getShopPayload(student.studentId));
      return;
    }

    await unlockAchievementBadges(student.studentId);
    const badge = await dbOne(
      `
        SELECT pb.id
        FROM profile_badges pb
        JOIN student_badges sb ON sb.badge_id = pb.id AND sb.student_id = $2
        WHERE pb.badge_code = $1 AND pb.active = TRUE
      `,
      [badgeCode, student.studentId],
    );

    if (!badge) {
      throw createHttpError(400, "Сначала откройте или купите эту плашку.");
    }

    await dbQuery("UPDATE students SET active_badge_id = $2 WHERE id = $1", [student.studentId, badge.id]);
    response.json(await getShopPayload(student.studentId));
  }),
);
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    const student = await getStudentByToken(request);
    const allowedStatuses = new Set(["focused", "chill", "grinding", "on_fire", "need_coffee", "ready_exam", "resting"]);
    const profileStatus = cleanText(request.body.profileStatus, 40);

    if (!allowedStatuses.has(profileStatus)) {
      throw createHttpError(400, "Выберите статус из списка.");
    }

    await dbQuery("UPDATE students SET profile_status = $2 WHERE id = $1", [student.studentId, profileStatus]);
    response.json(await buildAccountPayload(student.studentId, student.authToken || ""));
  }),
);
...
app.get(
  ...
  asyncRoute(async (request, response) => {
    const staff = await getStaffByToken(request);
    response.json({ source: "database", ...(await getStaffShopPayload(staff)) });
  }),
);
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    ...
    try {
      await client.query("BEGIN");
      const item = await client.query("SELECT * FROM shop_items WHERE item_code = $1 AND active = TRUE AND audience IN ('All', $2)", [itemCode, staff.role]);
      if (item.rowCount === 0) {
        throw createHttpError(404, "Предмет не найден.");
      }

      const owned = await client.query("SELECT 1 FROM staff_shop_items WHERE staff_role = $1 AND staff_id = $2 AND item_id = $3", [
        staff.role,
        staff.staffId,
        item.rows[0].id,
      ]);
      ...
      const balance = await getStaffPoints(client, staff);
      if (balance < item.rows[0].price_points) {
        throw createHttpError(400, "Недостаточно баллов.");
      }
...
app.post(
  "/api/staff/shop/equip",
  asyncRoute(async (request, response) => {
    const staff = await getStaffByToken(request);
    const itemCode = cleanText(request.body.itemCode, 80);
    const unequip = Boolean(request.body.unequip);
    const item = await dbOne("SELECT id, item_type AS \"itemType\" FROM shop_items WHERE item_code = $1 AND active = TRUE AND audience IN ('All', $2)", [itemCode, staff.role]);

    if (!item) {
      throw createHttpError(404, "Предмет не найден.");
    }

    const owned = await dbOne("SELECT 1 FROM staff_shop_items WHERE staff_role = $1 AND staff_id = $2 AND item_id = $3", [
      staff.role,
      staff.staffId,
      item.id,
    ]);
    ...
    if (unequip) {
      await dbQuery("UPDATE staff_shop_items SET is_equipped = FALSE WHERE staff_role = $1 AND staff_id = $2 AND item_id = $3", [
        staff.role,
        staff.staffId,
        item.id,
      ]);
      ...
      await dbQuery(
        `
          UPDATE staff_shop_items ssi
          SET is_equipped = CASE WHEN ssi.item_id = $3 THEN TRUE ELSE FALSE END
          FROM shop_items i
          WHERE ssi.item_id = i.id
            AND ssi.staff_role = $1
            AND ssi.staff_id = $2
            AND i.item_type = $4
        `,
        [staff.role, staff.staffId, item.id, item.itemType],
      );
    }
...
async function addStaffPoints(client, staff, points, reason) {
  if (!staff?.role || !staff?.staffId || !points) {
    return;
  }

  await client.query("INSERT INTO staff_point_transactions (staff_role, staff_id, points, reason) VALUES ($1, $2, $3, $4)", [
    staff.role,
    staff.staffId,
    Number(points),
    reason,
...
async function getStaffPoints(client, staff) {
  const result = await client.query(
    "SELECT COALESCE(SUM(points), 0)::integer AS points FROM staff_point_transactions WHERE staff_role = $1 AND staff_id = $2",
    [staff.role, staff.staffId],
  );
  return Number(result.rows[0]?.points || 0);
}

async function getStaffShopPayload(staff) {
  const points = await dbOne("SELECT COALESCE(SUM(points), 0)::integer AS points FROM staff_point_transactions WHERE staff_role = $1 AND staff_id = $2", [
    staff.role,
    staff.staffId,
  ]);
  const items = await dbQuery(
    `
      ...
      FROM shop_items i
      LEFT JOIN staff_shop_items ssi
        ON ssi.item_id = i.id
       AND ssi.staff_role = $1
       AND ssi.staff_id = $2
      WHERE i.active = TRUE
        AND i.audience IN ('All', $1)
  ...
  return {
    pointsTotal: points?.points || 0,
    items: items.rows.map((item) => ({ ...item, isOwned: Boolean(item.isOwned), isEquipped: Boolean(item.isEquipped) })),
  };
}

function normalizeProfileShopItem(row) {
  return {
...
async function getStudentProfileItemsMap(studentIds = []) {
  ...
  const rows = await dbQuery(
    `
      SELECT
        ssi.student_id AS "studentId",
        i.item_code AS "itemCode",
        i.item_type AS "itemType",
        i.css_class AS "cssClass"
      FROM student_shop_items ssi
      JOIN shop_items i ON i.id = ssi.item_id
      WHERE ssi.is_equipped = TRUE
        AND ssi.student_id = ANY($1::int[])
        AND i.active = TRUE
    `,
    [ids],
  );
...
async function getStaffProfileItemsMap(staffRefs = []) {
  const refs = staffRefs
    .filter(Boolean)
    .map((ref) => ({ role: cleanText(ref.role, 20), staffId: Number(ref.staffId || ref.id || 0) }))
    .filter((ref) => ref.role && ref.staffId);
  const map = new Map(refs.map((ref) => [`${ref.role}:${ref.staffId}`, []]));
  ...
  refs.forEach((ref) => {
    params.push(ref.role, ref.staffId);
    conditions.push(`(ssi.staff_role = $${params.length - 1} AND ssi.staff_id = $${params.length})`);
  });

  const rows = await dbQuery(
    `
      SELECT
        ssi.staff_role AS "staffRole",
        ssi.staff_id AS "staffId",
        i.item_code AS "itemCode",
        i.item_type AS "itemType",
        i.css_class AS "cssClass"
      FROM staff_shop_items ssi
      JOIN shop_items i ON i.id = ssi.item_id
      WHERE ssi.is_equipped = TRUE
        AND i.active = TRUE
  ...
  rows.rows.forEach((row) => {
    const key = `${row.staffRole}:${Number(row.staffId)}`;
    if (!map.has(key)) {
      map.set(key, []);
    }
    map.get(key).push(normalizeProfileShopItem(row));
...
async function unlockAchievementBadges(studentId) {
  await dbQuery(
    `
      INSERT INTO student_badges (student_id, badge_id, source)
      SELECT DISTINCT $1, pb.id, 'achievement'
      FROM profile_badges pb
      JOIN course_enrollments ce ON ce.student_id = $1 AND ce.status = 'Active'
      WHERE pb.active = TRUE
        AND pb.badge_type = 'achievement'
        AND COALESCE(ce.progress_percent, 0) >= COALESCE(pb.unlock_percent, 0)
        AND (pb.course_id IS NULL OR pb.course_id = ce.course_id)
...
async function getShopPayload(studentId) {
  await unlockAchievementBadges(studentId);
  const result = await dbQuery(
    `
      SELECT
        i.item_code AS "itemCode",
  ...
  const badges = await dbQuery(
    `
      SELECT
        pb.badge_code AS "badgeCode",
        pb.badge_name AS "badgeName",
        pb.badge_type AS "badgeType",
        pb.price_points AS "pricePoints",
        pb.css_class AS "cssClass",
        pb.description,
        pb.unlock_percent AS "unlockPercent",
        (sb.badge_id IS NOT NULL) AS "isOwned",
        (s.active_badge_id = pb.id) AS "isEquipped"
      FROM profile_badges pb
      CROSS JOIN students s
      LEFT JOIN student_badges sb ON sb.badge_id = pb.id AND sb.student_id = $1
      WHERE pb.active = TRUE
        AND s.id = $1
      ORDER BY pb.badge_type DESC, pb.price_points, pb.unlock_percent NULLS LAST, pb.id
    `,
    [studentId],
  );
  return {
    source: "database",
    pointsTotal: points?.points || 0,
    items: result.rows.map((item) => ({ ...item, isOwned: Boolean(item.isOwned), isEquipped: Boolean(item.isEquipped) })),
    badges: badges.rows.map((badge) => ({ ...badge, isOwned: Boolean(badge.isOwned), isEquipped: Boolean(badge.isEquipped) })),
  };
}
...
async function buildStaffWorkspace(staff) {
  ...
  const corrections = await dbQuery(
    `
      SELECT
        ...
        l.title AS "lessonTitle",
        CASE
          WHEN hc.attempt_number = 2 THEN CONCAT('2-я работа над ошибками: ', ht.title)
          ELSE CONCAT('Работа над ошибками: ', ht.title)
        END AS "homeworkTitle",
        ht.description AS "homeworkDescription",
        ht.file_url AS "taskLink",
        ha.due_at AS "dueAt",
        hc.status AS "homeworkStatus",
        hc.id AS "homeworkSubmissionId",
        hc.file_url AS "submissionLink",
        hc.status AS "submissionStatus",
        hc.submitted_at AS "submittedAt",
        hc.score,
        hc.feedback_text AS "feedbackText",
        hc.feedback_text AS "teacherComment",
        hc.checked_by_teacher_id AS "checkedByTeacherId",
        hc.checked_at AS "checkedAt",
        r.id AS "curatorReviewId",
        r.rating AS "curatorRating",
        r.comment_text AS "curatorComment",
        r.created_at AS "curatorReviewCreatedAt",
      ...
      LEFT JOIN course_enrollments ce ON ce.id = hc.enrollment_id
      LEFT JOIN courses c ON c.id = COALESCE(ce.course_id, ht.course_id)
      LEFT JOIN teachers t ON t.id = c.teacher_id
      LEFT JOIN lessons l ON l.id = ht.lesson_id
      LEFT JOIN staff_resource_reviews r
        ON r.resource_type = 'Correction'
       AND r.resource_id = hc.id
       AND r.staff_role = $2
       AND r.staff_id = $3
      WHERE c.status = 'Active'
        AND hc.status <> 'Cancelled'
  ...
  const analysisHomeworks = await dbQuery(
    `
      SELECT
        TRUE AS "isCorrection",
        TRUE AS "needsHomeworkReview",
        hc.id AS "correctionId",
        hc.attempt_number AS "correctionAttempt",
        ha.id AS "homeworkAssignmentId",
        ht.id AS "homeworkTemplateId",
        ...
        l.lesson_number AS "lessonNumber",
        l.title AS "lessonTitle",
        CONCAT('Работа над ошибками 2: ', ht.title) AS "homeworkTitle",
        ht.description AS "homeworkDescription",
        ht.file_url AS "taskLink",
        ha.due_at AS "dueAt",
        hc.status AS "homeworkStatus",
  ...
  const reviewHomeworks = await dbQuery(
    `
      SELECT
        FALSE AS "isCorrection",
        TRUE AS "isReviewHomeworkAssignment",
        hra.id AS "reviewHomeworkId",
        hra.id AS "homeworkSubmissionId",
        ha.id AS "homeworkAssignmentId",
        ht.id AS "homeworkTemplateId",
        c.id AS "courseId",
        c.slug AS "courseSlug",
        ...
        l.lesson_number AS "lessonNumber",
        l.title AS "lessonTitle",
        CONCAT('ДЗ V.2.0: ', ht.title) AS "homeworkTitle",
        ht.description AS "homeworkDescription",
        COALESCE(hra.task_url, l.homework_review_homework_url) AS "taskLink",
        l.homework_review_url AS "homeworkReviewUrl",
        hra.status AS "homeworkStatus",
        hra.status AS "submissionStatus",
        hra.file_url AS "submissionLink",
        hra.submitted_at AS "submittedAt",
        hra.score,
        hra.feedback_text AS "feedbackText",
        hra.feedback_text AS "teacherComment",
        hra.checked_by_teacher_id AS "checkedByTeacherId",
        hra.checked_at AS "checkedAt",
        r.id AS "curatorReviewId",
        r.rating AS "curatorRating",
        r.comment_text AS "curatorComment",
        r.created_at AS "curatorReviewCreatedAt",
      ...
      LEFT JOIN course_enrollments ce ON ce.id = COALESCE(hra.enrollment_id, ha.enrollment_id)
      JOIN courses c ON c.id = COALESCE(ce.course_id, ht.course_id)
      LEFT JOIN teachers t ON t.id = c.teacher_id
      LEFT JOIN lessons l ON l.id = COALESCE(hra.lesson_id, ht.lesson_id)
      LEFT JOIN staff_resource_reviews r
        ON r.resource_type = 'HomeworkReviewAssignment'
       AND r.resource_id = hra.id
       AND r.staff_role = $2
       AND r.staff_id = $3
      WHERE c.status = 'Active'
        AND hra.status <> 'Cancelled'
        AND ${staff.role === "Teacher" ? "COALESCE(hra.teacher_id, ha.teacher_id, ce.teacher_id, c.teacher_id) = $1" : "COALESCE(ce.curator_id, c.curator_id) = $1"}
      ORDER BY c.id, s.last_name, s.first_name, l.lesson_number, hra.id
    `,
    reviewParams,
  );
  ...
  const correctionStats = await dbQuery(
    `
      SELECT
        TRUE AS "isCorrection",
        FALSE AS "needsHomeworkReview",
        ...
        SUM(CASE WHEN hc.status = 'Checked' THEN 1 ELSE 0 END)::integer AS "checkedTotal",
        SUM(CASE WHEN r.id IS NOT NULL AND hc.id IS NOT NULL THEN 1 ELSE 0 END)::integer AS "curatorReviewedTotal",
        MAX(r.id) AS "curatorReviewId",
        MAX(r.rating) AS "curatorRating",
        MAX(r.comment_text) AS "curatorComment"
      FROM homework_corrections hc
  ...
  const analysisStats = await dbQuery(
    `
      SELECT
        ...
        COUNT(hc.id)::integer AS "homeworkTotal",
        COUNT(DISTINCT hc.student_id)::integer AS "studentsTotal",
        COUNT(hc.id)::integer AS "submittedTotal",
        COUNT(hc.id)::integer AS "checkedTotal",
        SUM(CASE WHEN r.id IS NOT NULL AND hc.id IS NOT NULL THEN 1 ELSE 0 END)::integer AS "curatorReviewedTotal",
        MAX(r.id) AS "curatorReviewId",
        MAX(r.rating) AS "curatorRating",
        MAX(r.comment_text) AS "curatorComment"
  ...
  const reviewHomeworkStats = await dbQuery(
    `
      SELECT
        FALSE AS "isCorrection",
        FALSE AS "needsHomeworkReview",
        ...
        ht.id AS "homeworkTemplateId",
        CONCAT('ДЗ V.2.0: ', ht.title) AS "homeworkTitle",
        COUNT(hra.id)::integer AS "homeworkTotal",
        COUNT(DISTINCT hra.student_id)::integer AS "studentsTotal",
        SUM(CASE WHEN hra.status IN ('Submitted', 'Checked') THEN 1 ELSE 0 END)::integer AS "submittedTotal",
        SUM(CASE WHEN hra.status = 'Checked' THEN 1 ELSE 0 END)::integer AS "checkedTotal",
        SUM(CASE WHEN r.id IS NOT NULL AND hra.id IS NOT NULL THEN 1 ELSE 0 END)::integer AS "curatorReviewedTotal",
        MAX(r.id) AS "curatorReviewId",
        MAX(r.rating) AS "curatorRating",
        MAX(r.comment_text) AS "curatorComment"
      FROM homework_review_assignments hra
  ...
  const mappedReviewHomeworks = reviewHomeworks.rows.map(mapStaffHomeworkDates);
  const mappedReviewHomeworkStats = reviewHomeworkStats.rows.map(mapHomeworkStat);
  ...
  return {
    ...
    reviewHomeworks: mappedReviewHomeworks,
    ...
    reviewHomeworkStats: mappedReviewHomeworkStats,
    staffShop: await getStaffShopPayload(staff),
    notifications: buildStaffNotifications(
      staff,
      [...mappedHomeworks, ...mappedCorrections, ...mappedAnalysisHomeworks, ...mappedReviewHomeworks],
      [...mappedStats, ...mappedCorrectionStats, ...mappedAnalysisStats, ...mappedReviewHomeworkStats],
    ),
  };
}
...
function buildStaffNotifications(staff, homeworks, stats) {
  const statKey = (item) =>
    `${item?.isReviewHomeworkAssignment ? "review-homework" : item?.needsHomeworkReview ? "analysis" : item?.isCorrection ? "correction" : "homework"}:${Number(item?.homeworkTemplateId || 0)}`;
  const byTemplate = new Map(stats.map((stat) => [statKey(stat), stat]));

  if (staff.role === "Teacher") {
    return homeworks
      .filter((homework) => homework.submittedAt || (homework.needsHomeworkReview && homework.checkedAt))
      .sort((a, b) => new Date((b.needsHomeworkReview ? b.checkedAt : b.submittedAt) || 0) - new Date((a.needsHomeworkReview ? a.checkedAt : a.submittedAt) || 0))
      .slice(0, 8)
      .map((homework) => {
        const stat = byTemplate.get(statKey(homework)) || {};
        const isCorrection = Boolean(homework.isCorrection);
        const isReviewHomeworkAssignment = Boolean(homework.isReviewHomeworkAssignment);
        const correctionAttempt = Number(homework.correctionAttempt || 1);
        const needsHomeworkReview = Boolean(homework.needsHomeworkReview);
        return {
          notificationId: `${isReviewHomeworkAssignment ? "teacher-review-homework" : needsHomeworkReview ? "teacher-review-needed" : isCorrection ? "teacher-correction" : "teacher-submission"}-${homework.homeworkSubmissionId}`,
          type: isReviewHomeworkAssignment ? "ReviewHomeworkSubmitted" : needsHomeworkReview ? "HomeworkReviewRequired" : isCorrection ? "CorrectionSubmitted" : "HomeworkSubmitted",
          ...
          title: needsHomeworkReview
            ...
            : isReviewHomeworkAssignment
              ? `${homework.studentName} сдал ДЗ V.2.0`
              : isCorrection
          ...
          createdAt: needsHomeworkReview ? homework.checkedAt : homework.submittedAt,
          link: "#staff-homework",
  ...
  return homeworks
    ...
    .map((homework) => {
      ...
      const needsHomeworkReview = Boolean(homework.needsHomeworkReview);
      return {
        notificationId: `${isReviewHomeworkAssignment ? "curator-review-homework" : needsHomeworkReview ? "curator-review-needed" : isCorrection ? "curator-correction" : "curator-checked"}-${homework.homeworkSubmissionId}`,
        type: isReviewHomeworkAssignment ? "ReviewHomeworkChecked" : needsHomeworkReview ? "HomeworkReviewRequired" : isCorrection ? "CorrectionChecked" : "HomeworkChecked",
        tone: homework.curatorReviewId ? "success" : "warning",
        title: needsHomeworkReview
          ? `${homework.teacherName} отправил ДЗ на разбор`
          : isReviewHomeworkAssignment
            ? `${homework.teacherName} проверил ДЗ V.2.0`
            : isCorrection
              ? `${homework.teacherName} проверил ${correctionAttempt >= 2 ? "2-ю работу над ошибками" : "работу над ошибками"}`
              : `${homework.teacherName} проверил ДЗ`,
        text: `${homework.courseTitle} · Урок ${homework.lessonNumber}. ${homework.homeworkTitle}`,
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    ...
    try {
      ...
      await addStaffPoints(client, staff, 18, "Проверка домашнего задания");
      let correctionId = null;
      if (score < 5) {
        const correction = await client.query(
          `
            INSERT INTO homework_corrections (assignment_id, attempt_number, student_id, teacher_id, enrollment_id, required_score, status)
            VALUES ($1, 1, $2, $3, $4, $5, 'Required')
            ON CONFLICT (assignment_id, attempt_number) DO UPDATE SET
              teacher_id = EXCLUDED.teacher_id,
              enrollment_id = EXCLUDED.enrollment_id,
              required_score = EXCLUDED.required_score,
            ...
            RETURNING id
          `,
          [assignmentId, row.student_id, row.teacher_id || staff.teacherId, row.enrollment_id, score],
        );
        correctionId = correction.rows[0]?.id || null;
      }
      if (row.enrollment_id) {
        await refreshEnrollmentProgress(client, row.enrollment_id);
      }
      ...
      response.json({
        ...
        source: "database",
        homeworkAssignmentId: assignmentId,
        homeworkSubmissionId: row.submission_id,
        score,
        feedbackText,
        correctionRequired: score < 5,
        correctionId,
      });
    } catch (error) {
      await client.query("ROLLBACK");
...
app.post(
  "/api/staff/corrections/:id/review",
  asyncRoute(async (request, response) => {
    const staff = await getStaffByToken(request);
    if (staff.role !== "Teacher" || !staff.teacherId) {
    ...
    try {
      ...
      const correction = await client.query(
        `
          SELECT
            ...
            hc.status,
            hc.file_url,
            ht.lesson_id AS lesson_id,
            l.homework_review_url AS homework_review_url,
            l.homework_review_homework_url AS homework_review_homework_url,
            COALESCE(hc.teacher_id, ha.teacher_id, ce.teacher_id, c.teacher_id) AS teacher_id
          FROM homework_corrections hc
          JOIN homework_assignments ha ON ha.id = hc.assignment_id
          JOIN homework_templates ht ON ht.id = ha.template_id
          JOIN courses c ON c.id = ht.course_id
          ...
          LEFT JOIN course_enrollments ce ON ce.id = hc.enrollment_id
          WHERE hc.id = $1
            AND COALESCE(hc.teacher_id, ha.teacher_id, ce.teacher_id, c.teacher_id) = $2
          FOR UPDATE OF hc
        `,
        [correctionId, staff.teacherId],
      );
      ...
      await client.query(
        `
          ...
          WHERE id = $4
        `,
        [staff.teacherId, score, feedbackText, correctionId],
      );
      await addStaffPoints(client, staff, 12, "Проверка работы над ошибками");

      let nextCorrectionId = null;
      const correctionAttempt = Number(row.attempt_number || 1);
      const needsHomeworkReview = score < 5 && correctionAttempt >= 2;

      if (score < 5 && correctionAttempt < 2) {
        const nextCorrection = await client.query(
          `
            INSERT INTO homework_corrections (assignment_id, attempt_number, student_id, teacher_id, enrollment_id, required_score, status)
            VALUES ($1, $2, $3, $4, $5, $6, 'Required')
            ...
            RETURNING id
          `,
          [row.assignment_id, correctionAttempt + 1, row.student_id, row.teacher_id || staff.teacherId, row.enrollment_id, score],
        );
        nextCorrectionId = nextCorrection.rows[0]?.id || null;
      }

      if (needsHomeworkReview && row.lesson_id && row.homework_review_url && row.homework_review_homework_url) {
        await ensureReviewHomeworkAssignmentsForLesson(client, row.lesson_id, row.homework_review_homework_url);
      }
      ...
      response.json({
        ...
        source: "database",
        correctionId,
        homeworkAssignmentId: row.assignment_id,
        correctionAttempt,
        nextCorrectionId,
        nextCorrectionAttempt: nextCorrectionId ? correctionAttempt + 1 : null,
        needsHomeworkReview,
        score,
        feedbackText,
      });
...
app.post(
  "/api/staff/review-homeworks/:id/review",
  asyncRoute(async (request, response) => {
    const staff = await getStaffByToken(request);
    if (staff.role !== "Teacher" || !staff.teacherId) {
      throw createHttpError(403, "Проверять ДЗ V.2.0 может только преподаватель.");
    }

    const reviewHomeworkId = toInt(request.params.id, 0);
    const score = toInt(request.body.score, 0);
    const feedbackText = cleanText(request.body.feedbackText || request.body.comment, 2000) || "ДЗ V.2.0 проверено.";

    if (!reviewHomeworkId || score < 1 || score > 10) {
      throw createHttpError(400, "Оценка должна быть от 1 до 10.");
    }
    ...
    try {
      await client.query("BEGIN");
      const reviewHomework = await client.query(
        `
          SELECT
            hra.id,
            hra.homework_assignment_id,
            hra.student_id,
            hra.enrollment_id,
            hra.status,
            hra.file_url,
            COALESCE(hra.teacher_id, ha.teacher_id, ce.teacher_id, c.teacher_id) AS teacher_id
          FROM homework_review_assignments hra
          JOIN homework_assignments ha ON ha.id = hra.homework_assignment_id
          JOIN homework_templates ht ON ht.id = ha.template_id
          JOIN courses c ON c.id = ht.course_id
          LEFT JOIN course_enrollments ce ON ce.id = COALESCE(hra.enrollment_id, ha.enrollment_id)
          WHERE hra.id = $1
            AND COALESCE(hra.teacher_id, ha.teacher_id, ce.teacher_id, c.teacher_id) = $2
          FOR UPDATE OF hra
        `,
        [reviewHomeworkId, staff.teacherId],
      );

      if (reviewHomework.rowCount === 0) {
        throw createHttpError(404, "ДЗ V.2.0 недоступно этому преподавателю.");
      }

      const row = reviewHomework.rows[0];
      if (row.status === "Checked") {
        throw createHttpError(409, "Это ДЗ V.2.0 уже проверено.");
      }

      if (row.status !== "Submitted" || !row.file_url) {
        throw createHttpError(400, "Ученик ещё не прикрепил ссылку на ДЗ V.2.0.");
      }
      ...
      await client.query(
        `
          ...
          WHERE id = $4
        `,
        [staff.teacherId, score, feedbackText, reviewHomeworkId],
      );

      await addStaffPoints(client, staff, 14, "Проверка ДЗ V.2.0");
      if (row.enrollment_id) {
        await refreshEnrollmentProgress(client, row.enrollment_id);
      }
      await client.query("COMMIT");
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    ...
    const homeworkReviewHomeworkUrl = normalizeLink(request.body.homeworkReviewHomeworkUrl || request.body.homework_review_homework_url);
    ...
    try {
      ...
      if (lessonId) {
        const updated = await client.query(
          `
            ...
            SET lesson_number = $1,
                ...
                homework_review_homework_url = $8,
                status = 'Open',
                is_open = TRUE,
                updated_at = NOW()
            WHERE id = $9
            ...
            RETURNING id
          `,
          [lessonNumber, lessonTitle, topic, videoUrl, notesUrl, homeworkUrl, homeworkReviewUrl, homeworkReviewHomeworkUrl, lessonId, courseId],
        );
        ...
        const inserted = await client.query(
          `
            INSERT INTO lessons (course_id, lesson_number, title, topic, video_url, notes_url, homework_url, homework_review_url, homework_review_homework_url, status, is_open)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'Open', TRUE)
            RETURNING id
          `,
          [courseId, lessonNumber, lessonTitle, topic, videoUrl, notesUrl, homeworkUrl, homeworkReviewUrl, homeworkReviewHomeworkUrl],
        );
      ...
      if (homeworkReviewUrl && homeworkReviewHomeworkUrl) {
        await ensureReviewHomeworkAssignmentsForLesson(client, savedLessonId, homeworkReviewHomeworkUrl);
      }
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    ...
    if (!["Lesson", "Note", "Homework", "Stream", "Correction", "HomeworkReview", "HomeworkReviewAssignment"].includes(resourceType) || !resourceId || !rating || rating < 1 || rating > 10) {
      throw createHttpError(400, "Оценка должна быть от 1 до 10.");
    }
    ...
    try {
      ...
      if (result.rowCount > 0) {
        await addStaffPoints(client, staff, 10, `Кураторская оценка: ${resourceType}`);
      }
      await client.query("COMMIT");
    } catch (error) {
      await client.query("ROLLBACK");
...
async function assertReviewResourceVisible(staff, resourceType, resourceId) {
  ...
  if (resourceType === "Lesson") {
  ...
  } else if (resourceType === "HomeworkReviewAssignment") {
    query = `
      SELECT 1
      FROM homework_review_assignments hra
      JOIN homework_assignments ha ON ha.id = hra.homework_assignment_id
    ...
    query = `
      ...
      JOIN homework_assignments ha ON ha.id = hc.assignment_id
      JOIN homework_templates ht ON ht.id = ha.template_id
      LEFT JOIN course_enrollments ce ON ce.id = COALESCE(hc.enrollment_id, ha.enrollment_id)
      JOIN courses c ON c.id = ht.course_id
      WHERE hc.id = $1
        AND ${homeworkReviewScope}
        AND hc.status = 'Checked'
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    const student = await getStudentByToken(request);
    const receiverId = toInt(request.body.receiverId || request.body.studentId, 0);

    if (!receiverId || receiverId === student.studentId) {
      throw createHttpError(400, "Выберите ученика для заявки в друзья.");
    }

    const receiver = await dbOne("SELECT id FROM students WHERE id = $1", [receiverId]);
    if (!receiver) {
      throw createHttpError(404, "Ученик не найден.");
    }
    ...
    const reversePending = await dbOne(
      "SELECT id FROM student_friend_requests WHERE requester_id = $1 AND receiver_id = $2 AND status = 'Pending'",
    ...
    await dbQuery(
      `
        INSERT INTO student_friend_requests (requester_id, receiver_id, status, updated_at)
    ...
    response.status(201).json({ ok: true, source: "database", ...(await getStudentFriendsPayload(student.studentId)) });
  }),
);
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    const student = await getStudentByToken(request);
    const requestId = toInt(request.body.requestId, 0);
    const action = cleanText(request.body.action, 20);

    if (!requestId || !["accept", "decline"].includes(action)) {
      throw createHttpError(400, "Выберите действие по заявке.");
    }

    const friendRequest = await dbOne(
      "SELECT id, requester_id AS \"requesterId\", receiver_id AS \"receiverId\" FROM student_friend_requests WHERE id = $1 AND receiver_id = $2 AND status = 'Pending'",
      [requestId, student.studentId],
    );
    if (!friendRequest) {
      throw createHttpError(404, "Заявка не найдена или уже обработана.");
    }

    await withTransaction(async (client) => {
      if (action === "accept") {
        await client.query("UPDATE student_friend_requests SET status = 'Accepted', updated_at = NOW() WHERE id = $1", [requestId]);
        await client.query(
          `
            INSERT INTO student_friends (student_id, friend_id)
            VALUES ($1, $2), ($2, $1)
            ON CONFLICT DO NOTHING
          `,
          [student.studentId, friendRequest.requesterId],
        );
      } else {
        await client.query("UPDATE student_friend_requests SET status = 'Declined', updated_at = NOW() WHERE id = $1", [requestId]);
      }
    });
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    ...
    if (!requestId) {
      throw createHttpError(400, "Заявка не выбрана.");
    }

    await dbQuery("UPDATE student_friend_requests SET status = 'Cancelled', updated_at = NOW() WHERE id = $1 AND requester_id = $2 AND status = 'Pending'", [
      requestId,
      student.studentId,
    ]);
...
app.post(
  ...
  asyncRoute(async (request, response) => {
    const student = await getStudentByToken(request);
    const friendId = toInt(request.body.friendId, 0);

    if (!friendId) {
      throw createHttpError(400, "Друг не выбран.");
    }

    await withTransaction(async (client) => {
      await client.query("DELETE FROM student_friends WHERE (student_id = $1 AND friend_id = $2) OR (student_id = $2 AND friend_id = $1)", [
        student.studentId,
        friendId,
      ]);
      await client.query(
        "UPDATE student_friend_requests SET status = 'Cancelled', updated_at = NOW() WHERE status = 'Pending' AND ((requester_id = $1 AND receiver_id = $2) OR (requester_id = $2 AND receiver_id = $1))",
        [student.studentId, friendId],
      );
    });
    ...
    `
      SELECT
        ...
        (
          SELECT ch.id
          FROM chats ch
          WHERE ch.chat_type = 'StudentFriend'
            AND ((ch.student_id = $1 AND ch.friend_student_id = s.id) OR (ch.student_id = s.id AND ch.friend_student_id = $1))
          ORDER BY ch.id
          LIMIT 1
        ) AS "conversationId"
      ...
      WHERE s.id <> $1
        ...
        AND NOT EXISTS (
          ...
          WHERE sfr.status = 'Pending'
            AND ((sfr.requester_id = $1 AND sfr.receiver_id = s.id) OR (sfr.requester_id = s.id AND sfr.receiver_id = $1))
        )
...
function getChatShape(actor, targetRole, targetId) {
  ...
  if (actor.role === "Student" && targetRole === "Student") {
    return {
      chatType: "StudentFriend",
      studentId: Math.min(Number(actor.id), Number(targetId)),
      friendStudentId: Math.max(Number(actor.id), Number(targetId)),
      teacherId: null,
      curatorId: null,
      adminId: null,
      targetRole,
...
async function findOrCreateConversation(actor, targetRole, targetId) {
  ...
  const existing = await dbOne(
    `
      ...
      WHERE chat_type = $1
        ...
        AND friend_student_id IS NOT DISTINCT FROM $3
        AND teacher_id IS NOT DISTINCT FROM $4
        AND curator_id IS NOT DISTINCT FROM $5
        AND admin_id IS NOT DISTINCT FROM $6
      ORDER BY id
      LIMIT 1
    `,
    [shape.chatType, shape.studentId, shape.friendStudentId || null, shape.teacherId, shape.curatorId, shape.adminId],
  );
  ...
  const created = await dbOne(
    `
      INSERT INTO chats (student_id, friend_student_id, teacher_id, curator_id, admin_id, chat_type, target_role, title)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
      RETURNING id AS "conversationId"
    `,
    [shape.studentId, shape.friendStudentId || null, shape.teacherId, shape.curatorId, shape.adminId, shape.chatType, shape.targetRole, title],
  );
...
async function assertConversationAllowed(actor, conversationId) {
  const where =
    actor.role === "Student"
      ? "((student_id = $2 AND chat_type IN ('StudentTeacher', 'StudentCurator', 'StudentAdmin')) OR (chat_type = 'StudentFriend' AND (student_id = $2 OR friend_student_id = $2)))"
      : actor.role === "Teacher"
  ...
  const conversations = await dbQuery(
    `
      SELECT
        ...
        s.id AS "studentId",
        CONCAT(s.first_name, ' ', s.last_name) AS "studentName",
        fs.id AS "friendStudentId",
        CONCAT(fs.first_name, ' ', fs.last_name) AS "friendStudentName",
        t.id AS "teacherId",
        CONCAT(t.first_name, ' ', t.last_name) AS "teacherName",
        cu.id AS "curatorId",
        CONCAT(cu.first_name, ' ', cu.last_name) AS "curatorName",
